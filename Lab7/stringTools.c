...
float stringToFloat(const char *str) {
    ...
    // Reset errno to 0 before using strtof
    errno = 0;

    // converting into a float typed buffer
    const float buffer = strtof(str, &endChar);

    if (errno == ERANGE) {
        printf("Numeric conversion impossible for string \"%s\" : output outside of float range.\n", str);
    } else if (*endChar != '\0' && *endChar != '\n') {
        printf("conversion stopped at \"%s\" : input is not fully numeric\n", endChar);
    } else if (endChar == str) {
        printf("conversion stopped at \"%s\" : input is empty\n", endChar);
